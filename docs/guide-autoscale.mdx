...
The next chapters show how to configure access to AWS, then how to
configure parameters and behovior of spawned machines. At the end the
usage in job definitions is presented.

Details about configuration can be found in [the Kraken
docs](autoscale-in-cloud).
...
To be sure that dangling instances do not eat up our budget let's set
the options in Destruction section to 1 job and to 10 minutes. So a
...
has defined environments section where we are pointing to out
