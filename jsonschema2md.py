...
class Parser:
    ...
    def _construct_description_line(
        ...
        if "const" in obj:
            description_line.append(f"Equals `'{obj['const']}'`.")
        ...
        if "enum" in obj:
            enums = ', '.join(("`'%s'`" % e for e in obj['enum']))
            description_line.append(f"Must be one of: {enums}.")
...
#                description_line.append("Cannot contain additional properties.")
        ...
        if "default" in obj:
            ...
            if isinstance(dflt, str):
                dflt = "'%s'" % dflt
            description_line.append(f"Default: `{dflt}`.")
        if required:
            description_line.append(f"The field is *required*.")
    ...
    def _parse_object(
        ...
        description_line = self._construct_description_line(obj, required=required)
